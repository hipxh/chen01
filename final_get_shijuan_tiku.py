...
if __name__ == '__main__':
    k = '2042001408349'
    v = '19980512'
    infos = ['659661','659662','659663','659664']
    ...
    chongdingxiang_result = requests.post(url=get_MoodleSession_url,
                                          ...
                                          # 2020年5月11日21:36:41此处是陈骁用fiddle告知必须正确传参才能保证不会签名错误.吸取教训,不该偷懒
                                          headers=headers)
    time.sleep(3)
    ...
    for info in infos:
        ...
        html = BeautifulSoup(shijuan_result.text, "html.parser")
        q_texts = html.find_all('div', class_='qtext')
        shitis = ""
        for q in q_texts:
            shitis += (q.text + "###################################################\r")
        data = open(course_name+cmid+html.title.string + '.txt', 'w+')
